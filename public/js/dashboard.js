 // Edit modal
  ...
  let currentDeliveryStopIndex = null; // <-- multi-stop: which stop is being delivered
  ...
  // ---------------- Stops UI (Create)
  function hasStopsUI() { return !!(stopsWrap && (btnAddPickup || btnAddDelivery)); }
  ...
  // ---------------- Events (timeline) - UNIQUE (fix dubbele AANGEMAAKT)
  async function addEventUnique(shipmentId, eventType, note = null, stopIndex = null) {
    const supabaseClient = await ensureClient();

    // Haal laatste event op om duplicaten te voorkomen
    ...
    if (!lastErr && last && last[0]) {
      const prev = last[0];
      const sameType = prev.event_type === eventType;
      const sameStop = (prev.stop_index ?? null) === (stopIndex ?? null);
      const sameNote = (prev.note ?? null) === (note ?? null);

      // Als exact hetzelfde laatste event is: skip
      if (sameType && sameStop && sameNote) return;
    }
  ...
  async function updateStatus(shipment, newStatus, extra = {}, eventNote = null) {
    try {
      ...
      try { await addEventUnique(shipment.id, newStatus, eventNote, null); } catch {}
  ...
  // ---------------- Per-stop status update (multi-stop)
  async function updateStopStatus(shipment, stopIndex, newStatus, noteForEvent = null) {
    ...
    const label = `${st.type === "pickup" ? "Ophalen" : "Bezorgen"}: ${st.address}`;
    await addEventUnique(shipment.id, overall, noteForEvent || `Stop ${stopIndex + 1} • ${label}`, stopIndex);
  ...
  // ---------------- Delivered modal (signature + photos) - SINGLE + MULTI
  let drawing = false;
  ...
  function onPointerUp(e) { drawing = false; last = null; e.preventDefault(); }
  ...
  function openDeliveredModal(shipment, stopIndex = null) {
    ...
    currentDeliveryStopIndex = stopIndex; // null = single shipment or overall deliver
    ...
    let infoLine = `${escapeHtml(shipment.pickup_address || "")} → ${escapeHtml(shipment.delivery_address || "")}`;
    if (typeof stopIndex === "number" && stops[stopIndex]) {
      const st = stops[stopIndex];
      infoLine = `<b>Stop ${stopIndex + 1} (${st.type === "delivery" ? "Bezorgen" : "Ophalen"}):</b> ${escapeHtml(st.address)}`;
      ...
      infoLine = `${escapeHtml(firstP)} → ${escapeHtml(lastD)} <span class="small">(${stops.length} stops)</span>`;
    }

    if (modalShipmentInfo) {
      modalShipmentInfo.innerHTML = `<b>${escapeHtml(shipment.track_code)}</b><br/>${infoLine}`;
    }
  ...
  if (modalConfirm) {
    modalConfirm.addEventListener("click", async () => {
      ...
      try {
        ...
        const sigPath = `${base}/signature-${Date.now()}.png`;
        ...
        // MULTI-STOP: schrijf proof op die delivery stop + status AFGELEVERD
        if (typeof currentDeliveryStopIndex === "number") {
          const shipment = currentDeliveryShipment;
          const stops = shipment._stopsNorm || normalizeStopsFromDb(shipment);
          if (!stops[currentDeliveryStopIndex]) throw new Error("Stop bestaat niet.");
          ...
          const label = `Stop ${currentDeliveryStopIndex + 1} • Bezorgen: ${st.address}`;
          await addEventUnique(shipment.id, overall, note ? `${label} • ${note}` : label, currentDeliveryStopIndex);

          // Als door deze stop overall AFGELEVERD wordt: archiveer-knop blijft werken
          await loadShipments(currentUserId);
        } else {
          // SINGLE: schrijf proof op shipment velden
          await updateStatus(
  ...
  async function saveEditShipment() {
    ...
    try {
      // preserve statuses + proof where possible by index
      ...
      const overall =
        mergedStops.length > 2 ? computeOverallStatusFromStops(mergedStops) : (currentEditShipment.status || "AANGEMAAKT");
  ...
  async function createShipment() {
    ...
    if (!customer_name) return msg("Vul klantnaam in.");
    if (shipment_type === "overig" && !shipment_type_other) return msg("Vul bij 'overig' een type in.");
    ...
    if (!hasPickup || !hasDelivery) return msg("Voeg minimaal 1 ophaaladres én 1 bezorgadres toe.");
    ...
    try {
      ...
      if (r.error) return msg("Fout: " + r.error.message);

      // Unique event -> geen dubbel "AANGEMAAKT"
      await addEventUnique(r.data.id, "AANGEMAAKT", null, null);
  ...
  function renderStopStatusUI(shipment) {
    ...
    stops.forEach((st, idx) => {
      ...
      // Alleen "Afgeleverd" met bewijs op DELIVERY-stops
      if (st.type === "pickup") {
        ...
        btns.appendChild(mkBtn("Afgeleverd", () => openDeliveredModal(shipment, idx))); // <-- modal per stop
      }
  ...
  function renderShipmentCard(s) {
    ...
    if (!s.archived_at) {
      ...
      if (!isMultiStopShipment(s)) {
        ...
        actions.appendChild(mkBtn("Afgeleverd", () => openDeliveredModal(s, null)));
      } else {
  ...
  // ---------------- Routeplanner + Maps (kept minimal / safe)
  let map = null;
  ...
  async function planOptimalRoute() {
    try {
      ...
      if (!stops.length) return routeMsg("Geen actieve zendingen voor routeplanning.");

      // simpele volgorde: zoals ingevoerd (jouw greedy algoritme kan hier weer terug)
      ...
      if (routeListEl) {
        routeListEl.innerHTML = "<b>Stops:</b><br/>" + stops.map((s, i) => `${i + 1}. ${escapeHtml(s.label)}`).join("<br/>");
      }
      routeMsg(`Route klaar • ${stops.length} stops`);
    } catch (e) {
      console.error(e);
